...
-->
	...
	String actors = request.getParameter("actors")==null?"":request.getParameter("actors");	 //배우
	String showTm = request.getParameter("showTm")==null?"":request.getParameter("showTm");	 //상영시간
	String watchGradeNm = request.getParameter("watchGradeNm")==null?"":request.getParameter("watchGradeNm");//등급
	String genreNm = request.getParameter("genreNm")==null?"":request.getParameter("genreNm");	//장르
...
<body>
	...
	<c:if test="${not empty result.movieListResult.movieList}"><form action ="/KobisOpenApiSample/servlet" method="get">
	<c:forEach items="${result.movieListResult.movieList}" var="movie">
		<tr>
			<td><c:out value="${movie.movieNm }"/><c:out value="${movie.movieCd }"/>
			<input type="text" value="${movie.movieCd }" name="movieCd">
			</td>
			<td><c:out value="${movie.movieNmEn }"/></td><td><c:out value="${movie.prdtYear }"/></td><td><c:out value="${movie.openDt }"/></td>
	...
	</c:forEach><input type="submit" value="입력">
	</form>
	</c:if>
	</table>
...
String sql = "insert into CINEBOX_MOVIE(MOVIE_ID, MOVIE_SORT, MOVIE_TITLE, MOVIE_DIRECTOR, MOVIE_ACTOR, MOVIE_AGE_GRADE,"
			+"MOVIE_RUNNINGTIME, MOVIE_COUNTRY, MOVIE_CONTENT, MOVIE_STATUS,"
			+" MOVIE_ADMIN_ID) values(MOVIE_SEQ.nextval,?,?,?,?,?,?,?,?,?,?)";
psmt=con.prepareStatement(sql);
psmt.setString(1, genreNm); psmt.setString(2, (String)result.get(movieNm)); psmt.setString(3, directorNm);
psmt.setString(4, actors); psmt.setString(5, watchGradeNm); psmt.setString(6,showTm);
psmt.setString(7, repNationCd); psmt.setString(8, null); psmt.setString(9, null);  
